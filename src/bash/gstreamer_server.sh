...
gst-launch-1.0 -v -e v4l2src device=$1 -v !\
     'video/x-raw, format=(string)I420, width=(int)640, height=(int)480' !\
     omxh264enc profile=8 control-rate=2 bitrate=800000  ! 'video/x-h264, stream-format=(string)byte-stream' !\
     h264parse ! rtph264pay ! udpsink host=$2 port=$3 async=false
