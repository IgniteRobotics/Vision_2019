	...
	if cnts is not None and (len(cnts) > 0):
		...
		if c is not None and len(c) != 0:
			...
			if((sd.detect(c) == "rectangle") or (sd.detect(c) == "square")):
				...
				frame = cv2.circle(frame, (cPoints[0][0], cPoints[0][1]), 5, (0, 0, 0))
				# Top right (Red)
				frame = cv2.circle(frame, (cPoints[1][0], cPoints[1][1]), 5, (0, 0, 0))
				# Bottom right (white)
				frame = cv2.circle(frame, (cPoints[2][0], cPoints[2][1]), 5, (0, 0, 0))
				# Bottom left (yellow)
				frame = cv2.circle(frame, (cPoints[3][0], cPoints[3][1]), 5, (0, 0, 0))
