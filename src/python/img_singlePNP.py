...
# obstructed test
greenLower = (101,45,36) 
greenUpper = (110,255,167) 
...
def find_highest_Y_Pts(candidates):
	...
	return np.array((to_ret[0],to_ret[2],to_ret[3],to_ret[1]))
...
def pickTapePairs(contours, img):
	...
	print ('top contour:', top_contour)
	cv2.drawContours(img, [top_contour], 0, (0,255, 0), 3)

	
	return np.array(top_contour), img
...
if c is not None and len(c) != 0:
	...
	print ('top countour corners', np.asarray(cTop,dtype=np.float32))
	...
	_, rvec, tvec = cv2.solvePnP(obj_points, np.asarray(cTop,dtype=np.float32), zero_camera_matrix, zero_distort_matrix)
